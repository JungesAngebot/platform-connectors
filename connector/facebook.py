...
def update_video_on_facebook(video: VideoModel, registry: RegistryModel):
    ...
    if video.video_hash_code == registry.video_hash_code:
        log_info("Metadata of registry entry %s not changed, so no update needed." % registry.registry_id)
        return

    mapping = MappingModel.create_from_mapping_id(registry.mapping_id)
    try:
        get_metadata_url = API_URL + "/" + registry.target_platform_video_id + "?access_token="+str(mapping.target_id)+"&fields=description,content_category,content_tags,title"
        ...
        raise Exception("Error updating video of registry entry %s to facebook." % registry.registry_id, e)
